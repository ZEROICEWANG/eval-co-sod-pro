...
class Eval_thread():
    ...
    def load_dataset(self):
        ...
        for pred, gt in self.loader:
            if self.cuda:
                ...
                pred = trans(pred)
                gt = trans(gt)
            data_list.append([pred, gt])
        return data_list

    def Eval_mae(self):
        avg_mae, img_num = 0.0, 0.0
        for i, (pred, gt) in enumerate(self.data):
            mea = torch.abs(pred.clone() - gt.clone()).mean()
            self.data[i][0] = (pred - torch.min(pred)) / (torch.max(pred) -
    ...
    def Eval_fmeasure(self):
        ...
        for (pred, gt) in self.data:
            pred = pred.clone()
            gt = gt.clone()

            prec, recall = self._eval_pr_mat(pred, gt, 255)
    ...
    def Eval_Emeasure(self):
        ...
        for (pred, gt) in self.data:
            pred = pred.clone()
            gt = gt.clone()
            Em += self._eval_e_mat(pred, gt, 255)
            img_num += 1.0
        Em /= img_num
        return Em
    ...
    def _eval_e_mat(self, y_pred, y, num):
        y_pred = y_pred.view(1, -1).repeat(num, 1)
        y = y.view(1, -1).repeat(num, 1)
        thlist = torch.linspace(0, 1 - 1e-10, num).cuda().view(-1, 1).repeat(1, y.size()[1])

        y_pred_th = (y_pred >= thlist).float()
        fm = y_pred_th - y_pred_th.mean(axis=1, keepdim=True)
        gt = y - y.mean(axis=1, keepdim=True)
        align_matrix = 2 * gt * fm / (gt * gt + fm * fm + 1e-20)
        enhanced = ((align_matrix + 1) * (align_matrix + 1)) / 4
        score = enhanced.sum(axis=1) / (y[0, :].numel() - 1 + 1e-20)
        return score
    ...
    def _eval_pr_mat(self, y_pred, y, num):
        y_pred = y_pred.view(1, -1).repeat(num, 1)
        y = y.view(1, -1).repeat(num, 1)
        thlist = torch.linspace(0, 1 - 1e-10, num).cuda().view(-1, 1).repeat(1, y.size()[1])

        y_pred = (y_pred >= thlist).float()
        tp = (y_pred * y).sum(axis=1)
        prec, recall = tp / (y_pred.sum(axis=1) + 1e-20), tp / (y.sum(axis=1) + 1e-20)
        return prec, recall
...
if __name__ == '__main__':
    num = 3
    thlist = torch.linspace(0, 1 - 1e-10, num).cuda().view(-1, 1).repeat(1, num)
    print(thlist)
    a = thlist.mean(axis=1, keepdim=True)
    print(a)
    print(thlist - a)
    print(a.numel())
